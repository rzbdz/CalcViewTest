...
- 7.3 一个支持加减乘除优先级的转换中缀表达式为后缀表达式的测试类
...
- [ ] CalcController
- [ ] View 调用Controller
- [ ] Memory (可能不做这个功能了..) 的类属性方法,以及和M按钮之间的绑定
...
- [x] 整体 GUI
- [ ] Memory Frame 内容 (先不做这个)
- [ ] History Frame 内容 (先不做这个)
- [ ] GUI响应事件
  - [x] 文本框自适应
  - [ ] 菜单栏 (先不做这个)
    - [x] 内存
...
数据绑定一个栈(用Arraylist),或者就单单一个变量算了(如果这样后期去掉查看记录的功能)

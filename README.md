...
默认分支是master,更新master在哪都行.

开发时记得切到dev再写,

master保持最新完整子模块(以完整的事件/方法为一个子),更新的时候要解决冲突

single_pattern是之前的只有GUI类结构的版本(见下面的开发记录).



### 任务列表:

注:下划线部分正在开发,开发时可以加下划线避免重复干活
...
一个Controller绑定一个Model和一个View,这里的写法是, View 里面放一个Controller,给View调用Controller的方法,传参数(如这里Input都是字符串),然后Controller进行输入的过滤,确定要进行什么样的运算,再修改Model的属性和调用Model里的实际运算过程对Model修改,然后把结果返回给View 更新视图.

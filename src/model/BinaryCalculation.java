...
/**
 * 考虑一元和二元的逻辑不同,重命名为BinaryCalculation
 ...
 * 如果要添加运算种类,<br>
 * 要修改,能修改的部分:<br>
 * {@link BinaryCalculation#calcBinary(BigDecimal, BigDecimal, Operator)},
...
public class BinaryCalculation {
     ...
     */
    public void doCalculation(Deque<Item> infixStack) throws DivideByZeroException, InvalidExpressionException {
        this.infixStack = infixStack;
        this.result = suffixToResult(infixToSuffix(this.infixStack));
        System.out.println("BinaryCalculation - doCalculation: "+result.toString());
    }
     ...
     */
    private Deque<Item> infixToSuffix(Deque<Item> Infix) throws InvalidExpressionException {
                ...
                if (item instanceof Digit) {
                    ...
                    if (!operators.isEmpty()) {
                        ...
                        if (((Operator) item).isRightBracket()) {
                            ...
                            //复制CalculationForStringTestPro函数逻辑之后,这里暂时保留+-<*/<()优先级的写法.
     ...
     * @see BinaryCalculation#calcBinary
     */
    private BigDecimal suffixToResult(Deque<Item> suffixStack) throws DivideByZeroException, InvalidExpressionException {
        try {
    ...
    private BigDecimal calcBinary(BigDecimal decimalToCalc, BigDecimal decimalToBeCalc, Operator operation) throws DivideByZeroException {
        switch (operation.operator) {
            ...
            case POW:
                return decimalToBeCalc.pow(decimalToCalc.intValue());
            case LEFT_BRACKET:
            case RIGHT_BRACKET:
            default:
                //未考虑情况暂时返回被运算数
